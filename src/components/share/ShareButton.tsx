...
function buildReportUrl(
  ...
  if (typeof window !== "undefined") {
    ...
    if (schoolSlugs.length > 0) params.set("schools", schoolSlugs.join(","));
    if (otherParams) {
      Object.entries(otherParams).forEach(([k, v]) => params.set(k, v));
    }
...
export function ShareButton(props: ShareButtonProps) {
  ...
  const linkClass =
    "inline-flex items-center gap-2 text-sm font-sans text-charcoal hover:text-hermes focus:text-hermes transition-colors";
  const buttonClass =
    "inline-flex items-center gap-2 text-sm font-sans text-charcoal hover:text-hermes focus:text-hermes transition-colors cursor-pointer border-0 bg-transparent p-0";

  return (
    <div className="flex items-center gap-4 flex-wrap" role="group" aria-label="Share options">
      <span className="font-display text-sm uppercase tracking-wider text-charcoal">Share</span>
      <div className="flex items-center gap-4">
        <button type="button" onClick={handleCopy} className={buttonClass} aria-label="Copy link">
          {copied ? (
            <>
              <span className="inline-flex w-4 h-4 items-center justify-center rounded-full bg-green-500/20 text-green-600 text-xs">✓</span>
              Copied
            </>
          ) : (
            <>
        ...
        <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className={linkClass} aria-label="Share on WhatsApp">
        ...
        <a href={mailtoUrl} className={linkClass} aria-label="Share by email">
          <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
