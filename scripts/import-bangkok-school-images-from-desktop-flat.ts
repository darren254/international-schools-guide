/**
 * Import Bangkok school images from /Users/darren/Desktop/images-bangkok.
 * Files named like "School-Name-Bangkok_1.jpg", "School-Name-Bangkok_2.jpg" (or without "-Bangkok").
 * Converts to webp (profile 1600px, card 800px, photo1 1600px) and updates school-images.json.
 *
 * Run: npx tsx scripts/import-bangkok-school-images-from-desktop-flat.ts
 */
...
const SOURCE_BASE = "/Users/darren/Desktop/images-bangkok";
...
for (const s of BANGKOK_SCHOOLS) {
  const key = normalizeKey(s.name);
  nameToSlug.set(key, s.slug);
  if (key.endsWith(" bangkok")) {
    nameToSlug.set(key.slice(0, -8).trim(), s.slug);
  }
}
...
async function main() {
  ...
  for (const [fileBase, fileList] of bySchool) {
    ...
    let slug =
      nameToSlug.get(key) ??
      nameToSlug.get(key.replace(/\s+bangkok$/, "")) ??
      fileBaseToSlug[key];
    if (!slug) {
  ...
  if (skipped.length > 0) {
    console.warn("\nSkipped (no matching Bangkok school):", skipped.slice(0, 30).join(", "));
