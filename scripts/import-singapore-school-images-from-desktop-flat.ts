/**
 * Import Singapore school images from /Users/darren/Desktop/images-singapore.
 * Files named like "School-Name-Singapore_1.jpg", "School-Name-Singapore_2.jpg".
 * Converts to webp (profile 1600px, card 800px, photo1 1600px) and updates school-images.json.
 *
 * Run: npx tsx scripts/import-singapore-school-images-from-desktop-flat.ts
 */
...
const SOURCE_BASE = "/Users/darren/Desktop/images-singapore";
...
const fileBaseToSlug: Record<string, string> = {
  "sfa eduhearts international school singapore": "sfa-eduhearts-international-school",
  "yuvabharathi international school singapore": "yuva-bharathi-international-school",
};
...
async function main() {
  ...
  for (const [fileBase, fileList] of bySchool) {
    ...
    let slug =
      nameToSlug.get(key) ??
      nameToSlug.get(key.replace(/\s+singapore$/, "")) ??
      fileBaseToSlug[key];
    if (!slug) {
  ...
  if (skipped.length > 0) {
    console.warn("\nSkipped (no matching Singapore school):", skipped.slice(0, 20).join(", "));
